package com.mediamarkt.tests.login;
...
public class MediaMarktLoginTest extends BaseSetUp {

    //local variables
    private MediaMarktLoginPO<WebElement> loginPO = null;
    private MediaMarktHomePO<WebElement> homePO = null;
    private static final String DATA_FILE = "src\\test\\resources\\logindata\\MediaMarktLogin.json";
    ...
    protected void testMethodTeardown(ITestResult result) throws Exception {
        ...
        if (!driver.getCurrentUrl().contains("Login | MediaMarkt")) {
            loginPO.loadPage(Global_VARS.LOGIN_TARGET_URL, Global_VARS.TIMEOUT_ELEMENT);
        }
    }
    ...
    /**
     * tc001_loginCreds method to test login
     */
